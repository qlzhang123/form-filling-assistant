        ...
        try {
            ...
            let forms = formElements.map(form => {
                const inputs = Array.from(form.querySelectorAll('input, select, textarea')).map(input => {
                    return {
                        name: input.name || input.id || this.generateFieldName(input),
                        id: input.id || null,
                        type: input.type || 'text',
                        label: this.getElementLabel(input),
                        placeholder: input.placeholder || '',
                        required: input.hasAttribute('required'),
                        value: input.value || '',
                        options: this.getFieldOptions(input),
                        validation: this.getValidationRules(input)
                    };
                });
                ...
                return {
                    ...
                    id: form.id || null,
                    inputs: inputs.filter(input => input.type !== 'submit' && input.type !== 'button' && input.type !== 'hidden')
                };
            });

            // 查找不在表单中的孤立输入框
            const allInputs = Array.from(document.querySelectorAll('input, select, textarea'));
            const isolatedInputs = allInputs.filter(input => {
                // 排除已在表单中的、提交按钮、隐藏域等
            ...
            // 过滤掉没有任何输入框的表单
            forms = forms.filter(f => f.inputs.length > 0);

            sendResponse({
                success: true,
                forms: forms,
                totalCount: forms.reduce((sum, form) => sum + form.inputs.length, 0)
            });
        } catch (error) {
            sendResponse({
                success: false,
                message: `解析表单失败: ${error.message}`
            });
        }
    }
