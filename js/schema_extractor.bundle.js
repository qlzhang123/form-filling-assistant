...
// node_modules/@threvo/schema-extractor/src/detectors/detectPageType.js
var countMatches = ($2, selectors = []) => {
  return selectors.reduce((total, selector) => total + $2(selector).length, 0);
};
var hasHeroLikeContent = ($2) => {
  const heroCandidates = ["main", "section", ".hero", "#hero"];
  return countMatches($2, heroCandidates) > 0;
};
function detectPageType($2) {
  if (!$2) {
    return "unknown";
  }
  ...
  const searchScore = countMatches($2, searchSelectors);
  const productScore = countMatches($2, productSelectors);
  const navLinks = $2("nav a");
  if (productScore > 0 && searchScore > 0) {
    return "search";
  }
  if (productScore > 3 || $2('.price, [class*="price"]').length > 0 && productScore > 0) {
    return "product";
  }
  if (searchScore > 0 && $2("form").length > 0) {
    return "search";
  }
  if (hasHeroLikeContent($2) && navLinks.length > 3) {
    return "homepage";
  }
  return "unknown";
}

// node_modules/@threvo/schema-extractor/src/detectors/detectSearchInput.js
var escapeAttr = (value = "") => value.replace(/"/g, '\\"');
var toSelector = ($2, el) => {
  if (!el) return null;
  const $el = $2(el);
  const tag = (el.tagName || el.name || "input").toLowerCase();
  const id = $el.attr("id");
  if (id) return `#${id}`;
  ...
  if (type) return `${tag}[type="${escapeAttr(type)}"]`;
  return tag;
};
var ensureInputElement = ($2, node) => {
  const $node = $2(node);
  if ($node.is("input, textarea")) return $node;
  const nested = $node.find("input, textarea").first();
  return nested.length ? nested : null;
};
var fallbackByName = ($2) => {
  let match = null;
  $2("input").each((_, el) => {
    const name = ($2(el).attr("name") || "").toLowerCase();
    const placeholder = ($2(el).attr("placeholder") || "").toLowerCase();
    if (name.includes("search") || placeholder.includes("search")) {
      match = toSelector($2, el);
      return false;
    }
    return void 0;
  });
...
function detectSearchInput($2) {
  ...
  for (const selector of candidates) {
    const $matched = $2(selector).first();
    if ($matched.length) {
      const element = ensureInputElement($2, $matched);
      if (element) {
        return toSelector($2, element[0]);
      }
    }
  }
  return fallbackByName($2);
}

// node_modules/@threvo/schema-extractor/src/detectors/detectProducts.js
var escapeAttr2 = (value = "") => value.replace(/"/g, '\\"');
var buildSelector = ($2, el) => {
  if (!el) return null;
  const $el = $2(el);
  ...
  const dataId = $el.attr("data-testid") || $el.attr("data-test");
  if (dataId) return `${tag}[data-testid="${escapeAttr2(dataId)}"]`;
  return tag;
};
var uniquePush = (collection, value) => {
  if (!value || collection.includes(value)) return;
  collection.push(value);
};
var findFirstSelector = ($2, selectors) => {
  for (const selector of selectors) {
    const node = $2(selector).first();
    if (node.length) {
      return buildSelector($2, node[0]);
    }
  }
  return null;
};
function detectProducts($2) {
  const result = {
    product_cards: [],
    product_title: null,
    product_price: null,
  ...
  cardSelectors.forEach((selector) => {
    $2(selector).slice(0, 10).each((_, el) => uniquePush(result.product_cards, buildSelector($2, el)));
  });
  ...
  result.product_title = findFirstSelector($2, titleSelectors);
  result.product_price = findFirstSelector($2, priceSelectors);
  result.product_image = findFirstSelector($2, imageSelectors);
  return result;
}

// node_modules/@threvo/schema-extractor/src/detectors/detectButtons.js
var escapeAttr3 = (value = "") => value.replace(/"/g, '\\"');
var buildSelector2 = ($2, el) => {
  if (!el) return null;
  const $el = $2(el);
  const tag = (el.tagName || el.name || "button").toLowerCase();
  const id = $el.attr("id");
  if (id) return `#${id}`;
  const classes = ($el.attr("class") || "").split(/\s+/).filter(Boolean);
  if (classes.length) {
  ...
  const type = $el.attr("type");
  if (type) return `${tag}[type="${escapeAttr3(type)}"]`;
  return tag;
};
var uniquePush2 = (collection, value) => {
  if (!value || collection.includes(value)) return;
  collection.push(value);
};
function detectButtons($2) {
  const selectors = [];
  if (!$2) {
    return selectors;
  }
  ...
  buttonCandidates.forEach((selector) => {
    $2(selector).slice(0, 20).each((_, el) => uniquePush2(selectors, buildSelector2($2, el)));
  });
  return selectors;
}

// node_modules/@threvo/schema-extractor/src/detectors/detectForms.js
var escapeAttr4 = (value = "") => value.replace(/"/g, '\\"');
var buildSelector3 = ($2, el) => {
  if (!el) return null;
  const $el = $2(el);
  const tag = (el.tagName || el.name || "form").toLowerCase();
  const id = $el.attr("id");
  if (id) return `#${id}`;
  const classes = ($el.attr("class") || "").split(/\s+/).filter(Boolean);
  if (classes.length) {
  ...
  if (action) {
    const preview = escapeAttr4(action.slice(0, 25));
    return `${tag}[action*="${preview}"]`;
  }
  return tag;
};
var uniquePush3 = (collection, value) => {
  if (!value || collection.includes(value)) return;
  collection.push(value);
};
var resolveFormNode = ($2, el) => {
  const $el = $2(el);
  if ($el.is("form")) return $el;
  const parent2 = $el.closest("form");
  return parent2.length ? parent2 : $el;
};
function detectForms($2) {
  const selectors = [];
  if (!$2) {
    return selectors;
  }
  ...
  formCandidates.forEach((selector) => {
    $2(selector).slice(0, 10).each((_, el) => {
      const node = resolveFormNode($2, el);
      if (node && node.length) {
        uniquePush3(selectors, buildSelector3($2, node[0]));
      }
    });
  });
  return selectors;
...
// node_modules/@threvo/schema-extractor/src/detectors/detectLinks.js
var escapeAttr5 = (value = "") => value.replace(/"/g, '\\"');
var buildSelector4 = ($2, el) => {
  if (!el) return null;
  const $el = $2(el);
  const tag = (el.tagName || el.name || "a").toLowerCase();
  const id = $el.attr("id");
  if (id) return `#${id}`;
  const classes = ($el.attr("class") || "").split(/\s+/).filter(Boolean);
  if (classes.length) {
  ...
  if (href && href !== "#") {
    return `${tag}[href*="${escapeAttr5(href.slice(0, 30))}"]`;
  }
  return tag;
};
var uniquePush4 = (collection, value) => {
  if (!value || collection.includes(value)) return;
  collection.push(value);
};
function detectLinks($2) {
  const selectors = [];
  if (!$2) {
    return selectors;
  }
  $2("a[href]").slice(0, 30).each((_, el) => uniquePush4(selectors, buildSelector4($2, el)));
  return selectors;
}

// node_modules/@threvo/schema-extractor/src/extractSchema.js
var buildEmptySchema = () => ({
  page_type: "unknown",
  search_input: null,
  product_cards: [],
  product_title: null,
...
var isUrl = (value) => typeof value === "string" && /^https?:\/\//i.test(value.trim());
async function extractSchema(input) {
  const fallback = buildEmptySchema();
  ...
  try {
    let html3 = input;
    if (isUrl(input)) {
      const response = await axios_default.get(input, {
        timeout: 1e4,
        headers: {
          "User-Agent": "@threvo/schema-extractor (+https://github.com/threvo)"
        }
      });
      html3 = response.data;
    }
    const $2 = load(html3 || "");
    const products = detectProducts($2);
    return {
      page_type: detectPageType($2) || fallback.page_type,
      search_input: detectSearchInput($2),
      product_cards: products.product_cards || fallback.product_cards,
      product_title: products.product_title,
      product_price: products.product_price,
      product_image: products.product_image,
      buttons: detectButtons($2),
      forms: detectForms($2),
      links: detectLinks($2)
    };
  } catch (error) {
    console.warn("[extractSchema] Failed to build schema:", error.message);
    return fallback;
  }
}
var extractSchema_default = extractSchema;
export {
  extractSchema_default as default,
  extractSchema
};
